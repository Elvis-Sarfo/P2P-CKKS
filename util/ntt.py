        ...
        for logm in range(1, log_num_coeffs + 1):
            for j in range(0, num_coeffs, (1 << logm)):
                for i in range(1 << (logm - 1)):
                    ...
                    # Check if the even_index and odd_index are in the range of the result
                    # vector. If not, we skip the butterfly operation.
                    if index_even < num_coeffs and index_odd < num_coeffs:
