...
class Polynomial:
    ...
    def add(self, poly, coeff_modulus=None):
        ...
        result_degree = max(self.ring_degree, poly.ring_degree)
        poly_sum = Polynomial(self.ring_degree, [0] * result_degree)

        # if the coefficients are out or range, use 0 instead
        # poly_sum.coeffs = [self.coeffs[i] + poly.coeffs[i]
        #                    for i in range(result_degree)]
        # for i in range(result_degree):
        #     if i < self.ring_degree and i < poly.ring_degree:
        #         poly_sum.coeffs[i] = self.coeffs[i] + poly.coeffs[i]
        #     elif i < self.ring_degree:
        #         poly_sum.coeffs[i] = self.coeffs[i]
        #     elif i < poly.ring_degree:
        #         poly_sum.coeffs[i] = poly.coeffs[i]
        #     else:
        #         poly_sum.coeffs[i] = 0

        # poly_sum.coeffs = [self.coeffs[i] + poly.coeffs[i]
        #                    for i in range(poly_sum)]
        for i in range(result_degree):
            a = self.coeffs[i] if i < self.ring_degree else 0
            b = poly.coeffs[i] if i < poly.ring_degree else 0
            poly_sum.coeffs[i] = a + b
